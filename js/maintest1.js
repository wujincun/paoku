...
var paoku ={
    ...
    collisionTest:function () {
        var _this = this;
        for (var i = 0; i < _this.intCons.blockList.length; i++) {
            if (_this.intCons.blockList[i] && _this.intCons.blockList[i].isShow) {
                var blockItem = _this.intCons.blockList[i],
                //小人中心点坐标
                    runnerHoriCenterCord = [_this.intCons.runner.positon[0] + _this.intCons.runner.size[0] / 2, _this.intCons.runner.positon[1] + _this.intCons.runner.size[1] / 2],
                //障碍物中心点坐标
                    blockHoriCenterCord = [blockItem.left + blockItem.width / 2, blockItem.top + blockItem.height / 2];

                if (Math.abs(runnerHoriCenterCord[0] - blockHoriCenterCord[0]) < (_this.intCons.runner.size[0] + blockItem.width) / 2 && Math.abs(runnerHoriCenterCord[1] - blockHoriCenterCord[1]) < (_this.intCons.runner.size[1] + blockItem.height) / 2) {
                    /*this.handleCollision(blockItem.type);
                    _this.intCons.blockList[i] = null;*/
                    //碰上就死掉的操作
